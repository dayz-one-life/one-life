...
- Report an objectionable avatar or block a player, from any dossier. A reported avatar is
  hidden straight away while it is reviewed, and shows the usual silhouette rather than a broken
  image while it waits. Reporting requires a verified gamertag, and a report names the exact
  image the reporter was looking at, so one report covers every account using that picture.
  Blocking hides someone's avatar from you and stops location sharing between you both ways —
  ...
  cannot hide it again, and says so rather than claiming it worked.
...
- The footer now links to One Life on Facebook, Discord, Reddit and X. The site previously
  pointed nowhere off-site, so anyone who wanted to follow the project or join the community
  server had no way to find it from the site itself.
- Search engines are now told that those four accounts and this site are the same One Life,
  rather than being left to guess. Previously the site published nothing at all about who runs
  it.
...
- **Test CI** (`rigging` → `.github/workflows/ci.yml`): a pnpm + turbo test job on Node 24 with a
  `postgres:16-alpine` service container. (Node 24, not the `engines.node >=20` floor: vitest's
  config loader natively `import()`s a `.ts` setup module, which needs the runtime's own type
  stripping — Node 20 can't, Node 22.18+/24 can. And `-alpine`, not glibc `postgres:16`: it matches
  the dev/prod image, whose musl/C collation orders a `_` in a callsign the way the friends
  `friendships_ordered` constraint and its tests assume.) rigging's per-service `database` key (Shipyard 0.9.0) emits
...
- **The map is now public.** Anyone can browse the terrain, town names and place search on any
