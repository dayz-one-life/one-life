...
describe("SurvivorRow", () => {
  ...
  test("hero row (rank 1) shows portrait, stat label, and the kills flourish", () => {
  ...
  test("compact row (rank 4) has no portrait", () => {
  ...
  test("unknown character renders no img (silhouette fallback is decorative)", () => {
    const { container } = render(<SurvivorRow rank={1} row={{ ...base, character: null }} />);
    expect(container.querySelector("img")).toBeNull();
  });
